...
import { parseFlagsArgv, shouldEscalate, runAsk, dispatchHeadline, dupAckFlag, parseOnOff, countBindingDebt, ensureEvidenceFileNodes, collectAskScopeIds } from "./cli-headlines.ts";
...
test("collectAskScopeIds: evidence の graph_context 表形式 (nodes/edges) でも落ちずに id を拾う", () => {
  // 回帰: graph_context は {nodes: Record<id,…>, edges: […]} の表であって配列ではない。
  // 旧実装は for…of で iterate しようとして TypeError → area_map が {error} に化けていた
  // (evidence まで自動エスカレーションした ask 全部で地図が消える)。
  const briefOut = {
    query: {
      matches: [
        {
          node: { id: "decision:s:a" },
          relations: [
            { relation: "refines", direction: "in", id: "decision:s:bare-id" },
            { relation: "led_to", direction: "in", node: { id: "investigation:s:embedded" } }
          ]
        }
      ]
    }
  ...
  const evidenceOut = {
    direct_evidence: [{ node: { id: "decision:s:b" } }],
    graph_context: {
      nodes: {
        "file:s:src/x.ts": { type: "File", title: "x.ts" },
        "concern:s:c": { type: "Concern", title: "c" }
      },
      edges: [{ relation: "documented_by", from: "decision:s:b", to: "file:s:src/x.ts" }]
    }
  };
  const ids = collectAskScopeIds(briefOut, evidenceOut);
  assert.deepEqual(
    [...ids].sort(),
    [
      "concern:s:c",
      ...
      "decision:s:bare-id",
      "file:s:src/x.ts",
      "investigation:s:embedded"
    ].sort()
  );
});

test("collectAskScopeIds: 欠損/異形入力に耐える (undefined / relations 非配列)", () => {
  assert.deepEqual(collectAskScopeIds(undefined, undefined), []);
  assert.deepEqual(collectAskScopeIds({ query: { matches: "not-an-array" } }, { graph_context: "nope" }), []);
  assert.deepEqual(
    collectAskScopeIds({ query: { matches: [{ node: { id: "ok:s:x" }, relations: "broken" }] } }, null),
    ["ok:s:x"]
  );
});
