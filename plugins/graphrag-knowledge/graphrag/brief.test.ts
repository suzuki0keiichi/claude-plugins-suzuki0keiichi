...
test("resume: open_goals は planned (残債) 優先の古い順 — 長命 active に枠を占有させない", () => {
  const graph = {
    nodes: [
      ...
      { id: "goal:s:roadmap", type: "Goal", title: "長期ロードマップ", state: "active", generated_at: "2025-01-01T00:00:00.000Z" },
  ...
  assert.equal(out.open_goals.planned_count, 2);
  assert.equal(out.open_goals.active_count, 1);
  assert.deepEqual(
    out.open_goals.planned_oldest_first.map((g) => g.id),
    ["goal:s:older", "goal:s:newer"],
    "planned だけが主枠 — 2025年からの active ロードマップより新しい残債が先に見える"
  );
  assert.equal(out.open_goals.oldest_active.id, "goal:s:roadmap", "active は最古1件と件数のみ");
