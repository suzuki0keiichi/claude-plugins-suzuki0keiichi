...
test("Goal: planned のまま stale なら suspect、新鮮/terminal/stateless は対象外", () => {
  const res = run([
    ...
    { id: "goal:s:old-active", type: "Goal", title: "進行中の目標", state: "active",
      generated_at: new Date(NOW - 100 * DAY).toISOString() },
    { id: "goal:s:roadmap-active", type: "Goal", title: "四半期以内に動きのあるロードマップ", state: "active",
      generated_at: new Date(NOW - 30 * DAY).toISOString() },
  ...
  assert.deepEqual(byId.get("goal:s:old-active")!.signals, ["stale-active-goal"],
    "active の停滞閾値は 90 日 — 100 日放置のロードマップは浮上する");
  assert.ok(!byId.has("goal:s:roadmap-active"),
    "active (ロードマップ語彙) は 14 日では催促しない — 長期ゴールを件数ノイズにしない");
  ...
  assert.equal(res.counts.goals_open, 4, "planned/active の総数 (fresh 含む)");
});
...
test("settled-premise: 現役 Decision premise が混在しても Goal 前提が全て terminal なら発火する (永久沈黙の解消)", () => {
  const graph = {
    nodes: [
      { id: "constraint:s:shadow", type: "Constraint", title: "Y は X まで信じられない" },
      { id: "goal:s:x", type: "Goal", title: "X", state: "achieved" },
      { id: "decision:s:live", type: "Decision", title: "現役の論理前提" },
      { id: "risk:s:r", type: "Risk", title: "state を持たない Risk" }
    ],
    edges: [
      { id: "e1", type: "has_premise", from: "constraint:s:shadow", to: "goal:s:x" },
      { id: "e2", type: "has_premise", from: "constraint:s:shadow", to: "decision:s:live" },
      { id: "e3", type: "has_premise", from: "constraint:s:shadow", to: "risk:s:r" }
    ]
  };
  const res = stocktake(graph, { vaultDir: "/x", staleDays: 14, now: NOW });
  const c = res.suspects.find((s) => s.id === "constraint:s:shadow");
  assert.ok(c, "Decision/Risk premise は未達性の前提ではない — Goal だけで判定する");
  assert.deepEqual(c!.signals, ["settled-premise"]);
});

test("settled-premise: abandoned を含む場合は abandoned-premise を併記 (破れの恒久化 — 消すな、転換せよ)", () => {
  const graph = {
    nodes: [
      { id: "constraint:s:shadow", type: "Constraint", title: "shadow" },
      { id: "goal:s:dead", type: "Goal", title: "誰もやらない", state: "abandoned" }
    ],
    edges: [{ id: "e1", type: "has_premise", from: "constraint:s:shadow", to: "goal:s:dead" }]
  };
  const res = stocktake(graph, { vaultDir: "/x", staleDays: 14, now: NOW });
  const c = res.suspects.find((s) => s.id === "constraint:s:shadow")!;
