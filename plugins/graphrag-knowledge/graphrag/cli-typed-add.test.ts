...
test("add-decision --sets-policy-for は Decision → 宛先の sets_policy_for エッジを作る", () => {
  const plan = buildAddDecisionPlan({
    ...
    setsPolicyFor: ["file:foo:a.ts", "stratum:foo:band1"]
  });
  ...
  assert.deepEqual(edges.map((e: any) => e.to).sort(), ["file:foo:a.ts", "stratum:foo:band1"]);
});
...
test("add-decision の文法違反フラグは throw (sets-policy-for に Decision 不可宛先)", () => {
  // sets_policy_for の宛先に Risk は許されない (File|Investigation|OK|Stratum|Vein|Pocket のみ)
...
test("add-risk --risks-in は Risk → 宛先の risks_in エッジを作る", () => {
  const plan = buildAddRiskPlan({
    ...
    risksIn: ["decision:foo:d1", "file:foo:a.ts", "pocket:foo:c1"]
  });
