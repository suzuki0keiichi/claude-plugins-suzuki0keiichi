...
test("索引再構築はロック解放後に走る (ネットワーク IO をクリティカルセクションから外す)", async () => {
  ...
  const observingIndex = async () => {
    lockHeldDuringIndex = existsSync(path.join(stateDir, "vault.lock"));
    seqParityDuringIndex = readSeq(stateDir) % 2;
    return { ok: true };
  };
  const res = await applyMutationToVault({
...
test("重複ゲート: suspect 無しなら status ok で出力に載る", async () => {
  ...
  assert.equal(res.applied, true);
  assert.deepEqual(res.duplicate_check, { status: "ok", suspects: [] });
});
