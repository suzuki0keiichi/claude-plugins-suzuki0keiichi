...
test("mid-merge + vault だけ staged → mutation は成功する (pathspec 無し commit)", async () => {
  ...
  assert.ok(
    stagedBeforeMutation.split("\n").every((f) => f.startsWith("vault/")),
...
/**
 * gitInitVault() の repo に実際の MERGE_HEAD (mid-merge state) を作る。vault 外の
 * foreign.txt で両ブランチが競合する変更をし、`git merge` を conflict のまま止める
 * (MERGE_HEAD が残り、foreign.txt は未解決 (unmerged) のまま)。
 */
function createConflictedMerge(repo: string): void {
  const base = currentBranch(repo);
  ...
  try {
    execFileSync("git", ["-C", repo, "merge", "-q", "feature"]);
  } catch {
    /* conflict expected: MERGE_HEAD が残る */
  }
}
...
// #10: 書き込み側の ask-trail 読みが GRAPHRAG_STATE_DIR を honor すること。
// suggestDeps.recentHitIds を DI せず (=既定経路: resolveAskStateDir 経由)、ask 側と
// 同じ共有解決関数で記録したヒットが precheck / premise_candidates に反映されるかを見る。
test("precheck/premise_candidates: GRAPHRAG_STATE_DIR 設定時、共有解決関数で記録したヒットを書き込み側が読める (#10)", async () => {
  const prevEnv = process.env.GRAPHRAG_STATE_DIR;
  const explicitStateDir = mkdtempSync(path.join(tmpdir(), "askstate-explicit-"));
  try {
    process.env.GRAPHRAG_STATE_DIR = explicitStateDir;
    const { vault } = gitInitVaultWithDecision();

    // ask 側と同じ resolveAskStateDir 経由で hit を記録する (GRAPHRAG_STATE_DIR が
    // 決め手になるので vaultDir 自体は解決に使われない)。
    recordAskHits("what is decision a", ["decision:s:a"], resolveAskStateDir(vault)!);

    const res = await applyMutationToVault({
      plan: decisionPlan("pc3", "precheck via shared resolver"),
      vaultDir: vault,
      // stateDir は敢えて渡さない (既定 = stateDirForVault(vault))。cacheDir(lock 用)は
      // 従来どおり vault 隣だが、ask-trail の読みは GRAPHRAG_STATE_DIR 側を見るはず。
      git: true,
      buildIndex: noopIndex,
      dupDeps: { loadIndex: () => ({ rows: [] }) },
      suggestDeps: {
        loadIndex: () => null,
        // recentHitIds は敢えて DI しない → 既定の resolveAskStateDir 経由を通す。
      },
    });

    assert.equal(res.applied, true);
    assert.equal(
      res.duplicate_check.precheck,
      undefined,
      "GRAPHRAG_STATE_DIR 越しに記録したヒットが見えるので advisory は不要"
    );
    assert.ok(
      res.suggestions.premise_candidates.some((c: any) => c.node_id === "decision:s:a"),
      "premise_candidates に直近ヒットの decision:s:a が載る"
    );
  } finally {
...
test("PR#41: crash 痕跡 (seq 奇数) + write journal 検出時は journal 記載の torn ファイルが吸収される", async () => {
  ...
  writeVaultWriteJournal(cacheDir, [path.join("Decision", "A.md")]);
...
test("再レビュー指摘3: crash 回復は journal 記載の torn path だけ吸収し、crash 以前からの利用者 WIP は吸収しない", async () => {
  ...
  writeVaultWriteJournal(cacheDir, tornDelta.written);
...
test("再レビュー指摘3: write journal は writeDelta の前に永続化され (触接予定パスを含む)、commit 成功後に消える", async () => {
  const { vault, stateDir } = gitInitVaultWithDecision();
  const cacheDir = path.join(stateDir, "cache");
  let journalAtWrite: { paths: string[] } | null = null;
...
test("applyMutationToVault: delete は tombstone 台帳に記録され mutation と同一コミットで確定する", async () => {
  const { repo, vault, stateDir } = gitInitVault();
  // 前段: decision:s:a (+ File + documented_by) を作る
  const r1 = await applyMutationToVault({
    plan: decisionPlan("a", "add a"),
    vaultDir: vault, stateDir, git: true, buildIndex: noopIndex,
  });
  assert.equal(r1.applied, true);
  assert.deepEqual(r1.tombstones, { recorded: 0, shards: [] }, "削除なし mutation は台帳に書かない");

  // 本段: a を消して b を後継として1コミットで置き換える
  const bPlan = decisionPlan("b", "replace a with b");
  const res = await applyMutationToVault({
    plan: {
      ...bPlan,
      nodes: [{ op: "delete", id: "decision:s:a" }, ...bPlan.nodes],
      successors: [{ old: "decision:s:a", new: "decision:s:b" }],
    },
    vaultDir: vault, stateDir, git: true, buildIndex: noopIndex,
  });
  assert.equal(res.applied, true);
  assert.equal(res.tombstones.recorded, 1);
  assert.equal(res.tombstones.shards.length, 1);

  // 台帳の中身: 後継とカスケードエッジ (a の documented_by) が残る
  const latest = latestTombstones(vault);
  const entry = latest.get("decision:s:a");
  assert.ok(entry, "削除された id が台帳から引ける");
  assert.equal(entry!.successor, "decision:s:b");
  assert.equal(entry!.type, "Decision");
  assert.ok(
    (entry!.cascaded_edges ?? []).some((e) => e.type === "documented_by" && e.from === "decision:s:a"),
    "カスケード削除されたエッジの全タプルが修復材料として残る"
  );

  // 同一コミットで確定している (作業ツリーに未コミット残骸が無い)
  const porcelain = execFileSync("git", ["-C", repo, "status", "--porcelain", "--", "vault"], { encoding: "utf8" }).trim();
  assert.equal(porcelain, "", "台帳ファイルも mutation と同じ commit に乗る (vault 配下に未コミット残骸なし)");
});

test("applyMutationToVault: successors の typo (plan で消さない old) は reject され何も書かれない", async () => {
  const { vault, stateDir } = gitInitVault();
  const head0 = vaultHead(vault);
  await assert.rejects(
    applyMutationToVault({
      plan: {
        ...decisionPlan("c", "bad successors"),
        successors: [{ old: "decision:s:not-deleted", new: "decision:s:c" }],
      },
      vaultDir: vault, stateDir, git: true, buildIndex: noopIndex,
    }),
    /Refusing to mutate invalid graph/
  );
  assert.equal(vaultHead(vault), head0, "HEAD は進まない (all-or-nothing)");
