...
test("parseCarvingConfig: reason / added 欠落は ERROR", () => {
  ...
  assert.match(errors[0], /reason required/);
  assert.match(errors[1], /added .*required/);
  assert.match(errors[2], /reason required/);
});
...
test("parseCarvingConfig: path 重複は ERROR、path 欠落エントリはエラーにして config から落とす", () => {
  ...
  assert.match(errors[0], /duplicate path/);
  assert.match(errors[1], /path required/);
