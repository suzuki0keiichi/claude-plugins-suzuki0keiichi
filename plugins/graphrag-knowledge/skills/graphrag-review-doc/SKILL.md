...
1. **逆引きで枠を組む**（method §2、pr-review と同じパイプライン）: 変更 File → 所属 Stratum/Pocket/Vein →
...
3. **概念デルタを拾う**（method §3）: 文書に「注釈」として載せるため、却下案再導入・層破り・Vein 規範vs実体・
...
- **影響を受ける構造**（横断軸）: 触れた **レイヤー(Stratum) / コンポーネント(Pocket) / 機能グループ(Vein)** を列挙し、
...
- **どこをいじったか**: 変更 File を、上の構造（Stratum/Pocket/Vein）に紐付けて示す。
