...
1. **Lifecycle sweep**: run `$CLI stocktake [--days <N=14>] [--vault <dir>]` (read-only, deterministic). Suspects carry `type` (`Investigation` | `Goal`) and signals: `stateless` | `stale-active` | `no-generated-at` | `progress-claim` | `stale-planned-goal` | `stale-active-goal`. If suspects are zero, say so and move to step 3.
2. **Adjudicate each suspect with corroboration** (the core of this skill):
   ...
   - **Goals (deferred work)** — three honest outcomes, corroborate before choosing: the work happened (git/code evidence) → `achieved`; the premise died or nobody will ever do it → `abandoned` (say so — a dead promise kept "planned" is a lie that resurfaces forever); still genuinely wanted → keep, and if it lacks `documented_by` wiring to the files where the debt lives, add it now (that wiring is what lets delta-check resurface it instead of relying on the next stocktake).
   - **Constraints with `settled-premise` (debt shadows)** — the "until X is done, Y cannot be trusted" warning whose premise Goal has settled. Corroborate that Y actually got fixed (the Goal being achieved claims it; verify against code/tests), then delete the shadow (with a 301 successor if a durable invariant replaced it). If Y is somehow still broken despite the Goal settling, that is new information — reopen honestly (new Goal + keep the shadow rewired to it) instead of leaving a stale premise.
