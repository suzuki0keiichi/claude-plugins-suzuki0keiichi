...
- `carve --root <repo> --system <name>` ── index → concern-suggest → policy-suggest → carving-check 連鎖。**索引直後は File 要約も Pocket/Stratum candidate 要約も「構成要素サマリ」の機械テンプレ (`summary_provisional`)。必ず読んで「意味」(何をする/何のため/どの関心) の要約に書き換え `summary_provisional` を外す**(残すと concern-suggest 拒否・carving-check ERROR。テンプレ要約は embedding を構成要素語で汚染し縦串検出を無意味化する)。Pocket/Stratum は indexer が canonical 地質名で出す。
...
| `concern-suggest` | embedding 近接で Vein (横断関心) 候補抽出 |
...
- **`File`**: 索引されたソースファイル。要約が機械テンプレのままだと `summary_provisional: true` が立つ — ファイルを読んで本物の要約に書き換えたら外す (retrieval/concern-suggest 品質の主レバー)。
- **知識 (8)**:
