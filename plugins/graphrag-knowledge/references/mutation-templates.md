...
- Put deep raw logs in a **ConversationChunk** (update-in-place on a fixed slug, high-value fragments only), wired to the Investigation via `discussed_in`. "Raw" ≠ verbatim: sanitize per the parent skill's §Content hygiene (no abusive language / insults / personal information — flush substance, not phrasing).
