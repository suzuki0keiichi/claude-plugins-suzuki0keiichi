...
- `cache/evidence-stale.jsonl` — `{ts, path}` per evidence-wired changed path. `ask` joins this against each delivered node's `generated_at` and attaches `evidence_stale` when the file moved after the node was last verified (detection is free; re-extraction stays lazy and human/LLM-adjudicated).
