...
- `ask --format md` — LLM-readable markdown digest of the same payload (matches with id / summary / state_note / evidence_stale / relations, then area_map / enforcement_debt / next_action). Default stays JSON (the full-fidelity contract); use md when you would otherwise reach for jq/python just to read the answer.
- `ask --lexical-only` — explicit degraded mode for embedding-endpoint outages: no index load, no embedding, keyword/alias/ngram matching only. This is NOT a silent fallback (semantic retrieval stays non-negotiable by default); the output carries `retrieval_mode.semantic:false` and a DEGRADED warning, and absence of hits is weak evidence in this mode.
- `ask` attaches `evidence_stale` to any delivered node whose File evidence changed after the node's `generated_at` (source: the commit-lane freshness ledger `cache/evidence-stale.jsonl`, written by worktree-lane delta-check). The note means "the canonical file moved after this node was last verified — verify against the file"; op:update the node to clear it. Two false-positive guards: a grace window (default 2h, `GRAPHRAG_EVIDENCE_STALE_GRACE_HOURS`) absorbs the routine "write back the node, then commit the code minutes later" ordering, and a best-effort git corroboration drops ledger entries whose change never materialized (reverted / uncommitted — file clean and no commit after `generated_at`).
...
- `evidence_paths` — the uncapped list of changed paths that are wired (via the same edge set) to at least one knowledge node. This is the raw material of the freshness ledger below.

**Commit-lane observation logs** (worktree input only — `--diff`/`--files` review runs of past ranges do not record; writes go to `cache/`, never the vault, and failures are silent):

- `cache/echo-log.jsonl` — every authority echo firing (alias / knowledge_id / occurrence count / sample paths). Without this there is no data to adjudicate which fingerprints are crying wolf. Aggregated by `stocktake` as `echo_stats`.
- `cache/evidence-stale.jsonl` — `{ts, path}` per evidence-wired changed path. `ask` joins this against each delivered node's `generated_at` and attaches `evidence_stale` when the file moved after the node was last verified (detection is free; re-extraction stays lazy and human/LLM-adjudicated; delivery applies the grace window + git corroboration described under ask, so same-session write-back ordering and reverted edits do not produce durable false ⚠).
...
When the commit lane has recorded echo firings (`cache/echo-log.jsonl`), the output also carries `echo_stats` — per-alias firing counts for the fingerprint stocktake. High-firing aliases are crying-wolf suspects (a fingerprint that fires on every legitimate use trains readers to ignore echoes), but the count alone is not proof: check the logged paths before dropping. Retire a fingerprint via `op:update` on the owning node's `aliases`.
