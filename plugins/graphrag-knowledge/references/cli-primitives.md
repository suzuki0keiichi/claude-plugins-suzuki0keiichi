...
## world-join — vault を world に参加させる

```sh
node graphrag/cli.ts world-join --world <dir>              # vault は GRAPHRAG_VAULT_DIR / 自動発見
node graphrag/cli.ts world-join --world <dir> --vault <dir>  # 明示指定
```

決定論的な 2 ステップ: ① world.json に自 vault のパスと `vault_slug` を追加 (重複時は no-op)、② `.graphrag/.env` に `GRAPHRAG_WORLD_DIR=<dir>` を書き込み (既存値は上書き)。world ディレクトリや world.json が存在しなければ新規作成する。VAULT.md が未作成の場合は警告、`vault_slug` が未設定の場合は cross-vault ref が解決できない旨を警告する。
...
- **world.json** (`<world-dir>/world.json`): vault dir へのポインタ列 (`{"vaults": ["<path>", {"path": "...", "slug": "..."}]}`)。`slug` は `vault_slug` (cross-vault ref の名前空間) で、xref resolver が world.json から直接引くために使う。説明や名前を持たせるとエラー (腐る電話帳防止) — 自己紹介は各 vault 側の `VAULT.md` に書く。
