...
1. **目的は QA でなくコントローラビリティ**。バグ検出は AI が diff レベルで人間に勝るので、ここで bug を探すのが主目的ではない。狙いは「大枠おまかせ、でも枠は超えるな」——人間が所有する概念層（知識軸 + 横断軸 Layer/Concern/Component）の意図を、コードが崩していないかを見る。
...
2. **着地点の逆引き**（横断軸）: 各変更 File が **どの Layer（層）/ Component（塊）/ Concern（脈）** に属するかを `ask` / `evidence` で引く。
   - 「UI Layer に変更が入った」等のルーティング信号はここで出る（変更 File → interface/画面層 Layer や UI Component）。
...
1. **影響圏の展開**: 変更 File を seed に `evidence --request "<変更Fileのpath>" --types File --limit 1 --neighbors 2` で引く。
   **neighbors は 2 が必須**（depth1 = 所属 Concern/Component/Layer と File 直宛の方針エッジ、depth2 = 同胞 File。1 では同胞まで届かない）。
   ...
   読むもの:
   - 所属 **Concern / Component / Layer** の `evidenced_by` File 群（depth2）= 同じ脈・塊・層の同胞 File。
...
4. **経路付きで選別**: 候補を機械的に全列挙しない（ノイズはレビューを儀式化させる）。裏取り（3）を通過した上で、
   ...
   例: エラー処理 Concern の規範に関わる変更が、脈に属する 3 File 中 1 File にしか触れていない → 残り 2 File を読み、
   リトライ欠落を確認して経路付きで提示。
5. **強制度は advisory 既定、ただし重大なら格上げして騒ぐ**（§3 の表と格上げ規則）: 集合差そのものは機械的でも
   ...
   （例: エラー処理 Concern の規範変更が片方のパスにしか入っていない / Constraint の前提を崩してセキュリティが緩む）は、
...
- **層破り（Layer）**: 変更後の import が依存の縦位置を逆走していないか（下位が上位に依存）。
- **Decision の暗黙の反故**: その領域を統べる Decision の意図を、明示せず崩していないか。
- **Concern の滲み出し / 規範 vs 実体**: 横断ノードの summary（= 規範・設計意図）に対し、変更後の実体（evidenced_by）がズレていないか。
  例: エラー処理 Concern の規範が「ユーザーに出す + リトライ」なのに、新パスが log だけ → Concern から外れている。
...
- **carving.json の免除追加**: diff に `.graphrag/carving.json` の変更が含まれる場合、**免除追加を findings に昇格して
  人間裁定**に回す（carving.json は Layer/Concern/Component と同格の人間所有の概念層。LLM は提案のみ可で、追記は user 承認後）。
...
| **advisory** | LLM 判断系: Concern 滲み出し・Decision 暗黙の反故・governance 退行・scope creep・**影響圏の触り残し（§2.5 網羅チェック）** | 表示のみ（文脈中の言及。網羅チェックは独立セクション） |

**段は検出経路ごとの既定値であって上限ではない（格上げ可・格下げ不可）**: advisory 帯の所見でも、統べる
Constraint / Concern 規範 / Risk と交差して「実害が出る（壊れる・穴が開く・前提が崩れる）」と判断できるなら
