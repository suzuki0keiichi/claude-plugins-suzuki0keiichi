...
- **Concern は距離で出さない**: 横断 = 概念。**LLM がコードベース全体を俯瞰して概念的にモデリングする**のが主役。`vein-hint` (embedding 近接クラスタリング) は LLM のモデリング後の盲点チェック用。
