...
`graphrag/schema.ts` の `NODE_TYPES`。軸2(横断構造)は地質メタファーで命名する。
旧 engineering 名 (Layer/Concern/Component) は alias として残し `canonicalType` が正規化する。
...
### 横断系(crosscut、地質メタファー)

地層を掘る比喩で、File 構造から独立に生き残るべき構造を3種に分ける:

- **Stratum (地層、旧 Layer)**: 水平に積もる層 = アーキテクチャ層。「どの層か」(縦の位置)。
- **Pocket (鉱塊、旧 Component)**: 局所に凝集した塊 = 構造的まとまり(パッケージ/モジュール根)。「どこにあるか」(構造)。
- **Vein (鉱脈、旧 Concern)**: 層や塊を貫いて走る筋 = 横断的関心事。File 構造とも層とも独立に生きるアーキ意図。
...
## なぜ Stratum ≠ Vein ≠ Pocket か

地質の比喩がそのまま切り分けの軸になる:

- **Pocket** (鉱塊) は「どこにあるか」(構造)。局所に凝集した塊。
- **Stratum** (地層) は「どの層か」(縦の位置)。水平に積もる層。
- **Vein** (鉱脈) は「何を横断的に気にしているか」(構造にも層にも縛られない意図)。層や塊を貫いて走る。
...
- `evidenced_by`: crosscut(Stratum/Vein/Pocket) → File のみ。意図は必ず実体に接地。
