...
Vein は構造距離では出ない (= Pocket と違って import グラフだけで決まらない)。
本質は意味判断で、最終確定は LLM 判断が担う。ただし、機械シグナルで「縦串候補」を
絞り込めるので、Vein carving の起点として下記 2 種類を併用する:

**構造シグナル (indexer 出力)**: `File.cross_component_in_degree`

- 各 File に「自分を import している distinct Pocket の数」が付与される
  (indexer の Vein candidate 補助シグナル、index-codebase.ts で計算)。
- これが 2 以上のファイルは「縦串候補」。複数 Pocket を貫いて参照されているので、
  Vein の動機が走っている可能性。
- 例: `core/logger.ts` が 4 Pocket から import → 観測性 Vein の縦串。
  `shared/validators.ts` が 3 Pocket から → input-validation Vein の縦串。
- ただし: 共通型 (`shared/types.ts`) や composition root (`server/services.ts`) も
  高 in-degree になる。これらは Vein ではなく allowed-orphan / 共通インフラ。
  機械シグナルだけで Vein と決めつけない。

**意味シグナル (機械抽出)**: embedding 近接クラスタリング

- 構造シグナルで拾えない Vein (= 規約・運用ルール系、特定ファイルへの集中 import が
  ないもの) も存在する。例: 「エラー分類とリトライ」は分類ロジックと retry スケジューラ
  が別ファイルで、必ずしも同じ縦串を共有しない。「自動更新」のようなレイヤー縦断
  機能セットも同様で、import の縦串ではなく意味の共有で繋がる。
- このタイプは `node graphrag/cli.ts concern-suggest` で機械抽出する。各 File の embedding
  ベクトル間 cosine similarity で k-NN graph を構築し、異 Pocket 所属の意味的
  近接ファイル群を Union-Find でクラスタ化。各クラスタを Vein candidate JSON
  として LLM に提示する (member_files / spanning_components / theme_words 入り)。
- 推奨パラメータ: `--threshold 0.92 --knn 1 --min-cluster 3 --min-span 2`
  (giant component 抑制のため k-NN graph を採用、threshold が低いと連結爆発する)。

**機械シグナルで拾えないもの**: 多技術スタック横断の機能セット

- 例: LUKS は luks-manager (TypeScript) / routes/luks (Express) / LuksPasswordDialog
  (React) / install-luks.sh (Bash) / unlock-luks@.service (systemd) / 99-luks-unlock.rules
  (udev) と 6 技術スタックを跨ぐ。embedding 距離が遠く意味シグナルでは束ねられない。
- path / ファイル名のテーマ語 (例: `luks` を含む) で拾うのが筋。これは carving 時に
  LLM が path を見て手動で対応する (機械化候補だが現状未実装)。

### Vein の命名指針 (抽象語で)

機械抽出された候補に名前を付ける時、機能名そのまま (例: `luks-encryption`) ではなく
**1 段抽象化した概念名**を優先する。
...
**起点として使う順序**:
1. `node graphrag/cli.ts concern-suggest` で意味シグナル candidate を取得 (最低限の機械抽出)。
2. `cross_component_in_degree ≥ 2` の File 一覧を取る (構造シグナル、補完用)。
3. path テーマ語が示す多技術スタック機能セット (LUKS 型) を手動で識別。
4. 各候補に対し、単一動機原則と横断条件 (≧2 Pocket) を確認、1 段抽象の命名で確定。
